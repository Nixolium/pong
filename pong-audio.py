"""
    ...
    > python3 pong-audio.py player --host_ip 10.150.132.220 --host_port 5005 --player_ip 128.135.204.216 --player_port 5007
...
sinewave1 = SineWave(pitch=0, pitch_per_second=100,
                     decibels=-15, decibels_per_second=100)
sinewave2 = SineWave(pitch=0, pitch_per_second=100,
                     decibels=-15, decibels_per_second=100)
...
def on_receive_ball(address, *args):
    ...
    if (mode == 'p1'):
        sinewave1.play()
        vol = num_to_range(ball_x, 0, 800, -5, -100)
        sinewave1.set_volume(vol)
        pitch = num_to_range(abs(paddle_1 - ball_y), 0, 450, 20, -10)
        sinewave1.set_pitch(pitch)

    if (mode == 'p2'):
        sinewave2.play()
        vol = num_to_range(ball_x, 800, 0, -5, -100)
        sinewave2.set_volume(vol)
        pitch = num_to_range(abs(paddle_2 - ball_y), 0, 450, 20, -10)
...
def on_receive_powerup(address, *args):
    ...
    if (args[0] == 3):
        if (mode == 'p1'):
            say("Big paddle is ready. Say go big to use.")
    if (args[0] == 4):
        if (mode == 'p2'):
            say("Big paddle is ready. Say go big to use.")
...
def on_receive_p1_bigpaddle(address, *args):
    ...
    if (mode == 'p1'):
        playsound('let me at them.wav', False)
...
def listen_to_speech():
    ...
    while not quit:
        ...
        try:
            ...
            if recog_results == "play" or recog_results == "start" or recog_results == "unpause" or recog_results == "continue":
                client.send_message('/g', 1)
            if recog_results == "level one" or recog_results == "easy":
                client.send_message('/l', 1)
            if recog_results == "level two" or recog_results == "hard":
                client.send_message('/l', 2)
            if recog_results == "level three" or recog_results == "insane":
                client.send_message('/l', 3)
            if recog_results == "pause":
                client.send_message('/g', 0)
            if recog_results == "go big" or recog_results == "big paddle" or recog_results == "power up" or recog_results == "ultimate" or recog_results == "activate ultimate":
...
def sense_microphone():
    ...
    while not quit:
        ...
        if (float(volume) > 0.00006):
            print("pitch "+str(pitch)+" volume "+str(volume))
            if (float(pitch) > 4000):
                # print("up")
                if (mode == "p1"):
                    if power_up_type != 1:
                        paddle_1 = min(paddle_1 + speed, 450)
                        client.send_message("/p", paddle_1)
                else:
                    if power_up_type != 2:
                        paddle_2 = min(paddle_2 + speed, 450)
                        client.send_message("/p", paddle_2)
            if (float(pitch) < 200):
                # print("up")
                if (mode == "p1"):
                    if power_up_type != 1:
                        paddle_1 = max(paddle_1 - speed, 0)
                        client.send_message("/p", paddle_1)
                else:
                    if power_up_type != 2:
                        paddle_2 = max(paddle_2 - speed, 0)
                        client.send_message("/p", paddle_2)
# -------------------------------------#
...
# do the same for p1
    ...
    def redraw_menu(self):
        ...
        self.start_label = pyglet.text.Label("press space to start", font_name=None, font_size=36,
                                             x=self.w.width//2, y=self.w.height//2, anchor_x='center', anchor_y='center')
        self.level_label = pyglet.text.Label("easy | hard | insane", font_name=None, font_size=24,
                                             x=self.w.width//2, y=self.w.height//2+100, anchor_x='center', anchor_y='center')
        if (self.m.level == 1):
            self.level_indicator_label = pyglet.text.Label(
                "------", font_name=None, font_size=24, x=self.w.width//2-105, y=self.w.height//2+80, anchor_x='center', anchor_y='center')
        elif (self.m.level == 2):
            self.level_indicator_label = pyglet.text.Label(
                "------", font_name=None, font_size=24, x=self.w.width//2-12, y=self.w.height//2+80, anchor_x='center', anchor_y='center')
        elif (self.m.level == 3):
            self.level_indicator_label = pyglet.text.Label(
                "---------", font_name=None, font_size=24, x=self.w.width//2+92, y=self.w.height//2+80, anchor_x='center', anchor_y='center')
...
class Window(pyglet.window.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.score_label = pyglet.text.Label(str(p1_score)+':'+str(p2_score), font_name=None,
                                             font_size=36, x=self.width//2, y=self.height//2, anchor_x='center', anchor_y='center')
        self.powerup_status_label = pyglet.text.Label(
            "status: ", font_name=None, font_size=16, x=self.width//2, y=self.height//8, anchor_x='center', anchor_y='center')
